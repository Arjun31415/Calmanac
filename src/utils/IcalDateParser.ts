...
export default function iCalDateParser(date) {
    ...
    const isLocalTime = date.length === 15;
    if (isLocalTime) return new Date(year, month, day, hour, minute, second);
 ...
 * @private
 */
function _validateFormat(date: String) {
    const d = date.split('');

    if (d.length !== 16 && d.length !== 15) return false;
    if (d[T_INDEX] !== 'T') return false;
    if (d[Z_INDEX] !== 'Z' && d.length == 16) return false;
